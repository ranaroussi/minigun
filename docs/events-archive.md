...
- **Bounded per beat, drained across ticks.** A single cron beat fetches at most a few hundred pages worth of events per send (the Worker caps at `MAX_PAGES_PER_BEAT` pages × 300 events) so one tick stays under Cloudflare's cron CPU budget. Larger sends are drained across consecutive ticks: the watermark is checkpointed after **every page**, so a beat that's interrupted (CPU limit, restart) resumes after the last finished page instead of restarting. While a send is still catching up, the beat advances the watermark **without** bumping `events_pulls_count` (which tracks the burst schedule, not cron ticks), so it stays first in the due-queue until it's caught up.
- **Writes are batched.** Each page resolves all its recipients in one query and folds its events into the rollups via a single batched transaction, keeping per-event overhead low even on six-figure sends.
