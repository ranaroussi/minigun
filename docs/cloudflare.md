...
| `ENGAGEMENT_STATS_ENABLED` | no   | `false`                            | Gates engagement retrieval — the events-pull cron behind the read surface (`/send/{id}/recipients`, `/send/{id}/clicks`, `/contacts/{id}/engagement`). Retrieval only; pruning is the separate `LIST_HYGIENE_AUTO_PRUNE_ENABLED`. Schema lands dormant; flip to `true` to start collecting. Deprecated alias: `EVENTS_ARCHIVE_ENABLED`. |
...
3. If `ENGAGEMENT_STATS_ENABLED=true`, walks the events-pull schedule for any send whose next beat is due. Each pull begins strictly after the previous pull's highest event timestamp (the send's `created_at` on the first pull) and folds every event into the per-`(send, contact)` `contact_message_engagement` rollup and the per-`(contact, list)` `contact_engagement` summary — no raw event rows are stored. The incremental watermark means each event is seen once, so counters stay correct without a dedup ledger. Schedule is burst-then-daily (`+0`, `+1h`, `+6h`, `+24h`, then daily for 30 days). No-op when the flag is off.
